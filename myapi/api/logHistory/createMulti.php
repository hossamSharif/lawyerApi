<?php 
 ...
 // $values = string ;
    ...
    foreach($data2 as $col){
     // print_r($col['pay_ref']);
      ...
      array_push($res , array(
        ...
        "datee" => $col['datee'],
        "typee" => $col['typee'] ,  
        "logToken" => addslashes($col['logToken']) ,
    ...
    $values = '';
     for($i = 0; $i < $max;$i++){
       if ($i == 0) { 
        $values = '('.$res[$i]['id'].',"'.$res[$i]['logRef'].'","'.$res[$i]['userId'].'","'.$res[$i]['typee'].'","'.$res[$i]['datee'].'","'.$res[$i]['logStatus'].'","'.$res[$i]['logToken'].'","'.$res[$i]['yearId'].'","'.$res[$i]['store_id'].'")';
       }
      elseif ( $i == $max-1) { 
        $values = ''.$values.',('.$res[$i]['id'].',"'.$res[$i]['logRef'].'","'.$res[$i]['userId'].'","'.$res[$i]['typee'].'","'.$res[$i]['datee'].'","'.$res[$i]['logStatus'].'","'.$res[$i]['logToken'].'","'.$res[$i]['yearId'].'","'.$res[$i]['store_id'].'")';
