<?php 
  ...
  $services = new PerchOrder_details($db);
 ...
 // $values = string ;
    ...
    $values = '';
     for($i = 0; $i < $max;$i++){
       if ($i == 0) { 
        $values = '('.$res[$i]['id'].',"'.$res[$i]['pay_ref'].'","'.$res[$i]['item_name'].'","'.$res[$i]['pay_price'].'",'.$res[$i]['quantity'].',"'.$res[$i]['tot'].'","'.$res[$i]['store_id'].'",'.$res[$i]['item_id'].',"'.$res[$i]['dateCreated'].'","'.$res[$i]['perch_price'].'")';
       }
      elseif ( $i == $max-1) { 
        $values = ''.$values.',('.$res[$i]['id'].',"'.$res[$i]['pay_ref'].'","'.$res[$i]['item_name'].'","'.$res[$i]['pay_price'].'",'.$res[$i]['quantity'].',"'.$res[$i]['tot'].'",'.$res[$i]['store_id'].','.$res[$i]['item_id'].',"'.$res[$i]['dateCreated'].'","'.$res[$i]['perch_price'].'")' ;
