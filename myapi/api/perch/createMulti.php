<?php 
 ...
 // $values = string ;
    ...
    $values = '';
     for($i = 0; $i < $max;$i++){
       if ($i == 0) { 
        $values = '('.$res[$i]['pay_id'].',"'.$res[$i]['pay_ref'].'","'.$res[$i]['store_id'].'","'.$res[$i]['tot_pr'].'","'.$res[$i]['pay'].'","'.$res[$i]['changee'].'","'.$res[$i]['pay_date'].'","'.$res[$i]['pay_method'].'",'.$res[$i]['cust_id'].',"'.$res[$i]['discount'].'","'.$res[$i]['pay_time'].'","'.$res[$i]['user_id'].'","'.$res[$i]['payComment'].'","'.$res[$i]['nextPay'].'","'.$res[$i]['yearId'].'","'.$res[$i]['companyId'].'")';
       }
      elseif ( $i == $max-1) { 
        $values = ''.$values.',('.$res[$i]['pay_id'].',"'.$res[$i]['pay_ref'].'","'.$res[$i]['store_id'].'","'.$res[$i]['tot_pr'].'","'.$res[$i]['pay'].'","'.$res[$i]['changee'].'","'.$res[$i]['pay_date'].'","'.$res[$i]['pay_method'].'",'.$res[$i]['cust_id'].',"'.$res[$i]['discount'].'","'.$res[$i]['pay_time'].'","'.$res[$i]['user_id'].'","'.$res[$i]['payComment'].'",,"'.$res[$i]['nextPay'].'","'.$res[$i]['yearId'].'","'.$res[$i]['companyId'].'"))';
