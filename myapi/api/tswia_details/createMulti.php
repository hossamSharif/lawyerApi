<?php 
 ...
 // $values = string ;
     ...
     // print_r($col['pay_ref']);
      ...
      $services->perch_price = $col['perch_price'] ;
       $services->yearId = $col['yearId'] ;
          $services->availQty = $col['availQty'] ;
       // $services->create();
      // Create post
    ...
    $values = '';
     for($i = 0; $i < $max;$i++){
       if ($i == 0) { 
        $values = '('.$res[$i]['id'].',"'.$res[$i]['pay_ref'].'","'.$res[$i]['item_name'].'","'.$res[$i]['pay_price'].'","'.$res[$i]['qtyReal'].'","'.$res[$i]['tot'].'","'.$res[$i]['store_id'].'",'.$res[$i]['item_id'].',"'.$res[$i]['dateCreated'].'","'.$res[$i]['perch_price'].'","'.$res[$i]['yearId'].'","'.$res[$i]['availQty'].'")';
       }
      elseif ( $i == $max-1) { 
        $values = ''.$values.',('.$res[$i]['id'].',"'.$res[$i]['pay_ref'].'","'.$res[$i]['item_name'].'","'.$res[$i]['pay_price'].'","'.$res[$i]['qtyReal'].'","'.$res[$i]['tot'].'",'.$res[$i]['store_id'].','.$res[$i]['item_id'].',"'.$res[$i]['dateCreated'].'","'.$res[$i]['perch_price'].'","'.$res[$i]['yearId'].'","'.$res[$i]['availQty'].'")' ;
