...
public function readByStoreByRef(){
  ...
  public function create() {
    ...
    $query = 'INSERT INTO ' .
      $this->table . '
    SET
    pay_ref = :pay_ref ,item_name = :item_name, pay_price = :pay_price, quantity = :quantity, tot = :tot, store_id = :store_id, dateCreated = :dateCreated, yearId = :yearId, tax = :tax, discount = :discount';
//    VALUES (NULL,:pay_ref,:item_name,:pay_price,:quantity,:tot,:store_id,:dateCreated)';
 ...
 public function readAllByModelAndBrand(){
    ...
    $query = 'SELECT 
          ...
          pay_details.yearId,
          pay_details.tax,
         pay_details.discount AS itemDiscount,
 
          IFNULL((SELECT sub_name FROM sub_accounts WHERE  sub_accounts.id =  pay.cust_id), 0) AS sub_name 
        FROM
         ' . $this->table . '  INNER JOIN pay ON pay_details.pay_ref = pay.pay_ref INNER JOIN items ON pay_details.item_id = items.id 
        WHERE 
